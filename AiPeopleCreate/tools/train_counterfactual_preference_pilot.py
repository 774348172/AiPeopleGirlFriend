"""Train a small DPO Adapter from the clean Gemma 4 base model."""
...
def _isolate_missing_mergekit() -> None:
    """TRL 0.24 imports optional merge callbacks even when DPO never uses them."""
    try:
...
DATA_ROOT = CREATE_ROOT / "训练数据" / "baiweixi_counterfactual_preference_pilot_v1"
...
class TextOnlyProcessor(ProcessorMixin):
    """Expose a VLM-compatible processor API while keeping this run text-only."""
    ...
    def __getattr__(self, name: str) -> Any:
        tokenizer = object.__getattribute__(self, "tokenizer")
    ...
    def __call__(self, *, images: Any = None, text: str, add_special_tokens: bool = False, **kwargs: Any) -> dict[str, Any]:
        if images is not None:
            raise RuntimeError("counterfactual preference pilot is text-only")
...
def token_audit(tokenizer: Any, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if max(lengths) > MAX_LENGTH:
        raise RuntimeError(f"preference example exceeds {MAX_LENGTH} tokens: {max(lengths)}")
    return {
        "rows": len(rows),
...
def main() -> int:
    ...
    if manifest["files"]["train.jsonl"] != sha256(TRAIN_PATH):
        raise RuntimeError("preference train hash differs from manifest")
    rows = load_jsonl(TRAIN_PATH)
    if len(rows) != 24 or {row["condition"] for row in rows} != {"known", "unknown"}:
        raise RuntimeError("unexpected preference pilot shape")
    ...
    dataset = Dataset.from_list(rows[:4] if args.smoke else rows)
