...
- SYS-09 正式检索验收通过（20/20 案例，P95 43ms，隔离零泄漏）。
- 质量集词表修订并重冻结（manifest `2bb53297…`，修复 5 个词表误杀项 + 修正 build_suite 训练路径），`validate_suite` 与测试 7 passed。
- 4 个人工长会话 v2 评审：3 pass + 1 issue（宽松口径），`human_sessions_completed=4` 落回流程。
- 完整 89 案例复验：判官（baiweixi-4b）零失败，34 个机器失败中 33 个为词表误杀、1 个真实失败（protagonist_job 编造）；生成阶段 245/245 全成功（M2 容错生效）。
- 长会话质量归因：早期预演用 177 字符简化锚导致答非所问，改用与正式 GAME_REPLY 一致的完整正典锚（3843 字符）+ 标准世界投影 + 温度 0.6 后全部恢复正常——确认模型多轮质量达标，非模型缺陷。

`SYS-11 / SYS-12` 系统稳定性已通过；P0 正式冻结剩余项为：质量集判定口径决策（判官为主、词表参考）、8GB 目标卡最终显存实测。`SYS-12` 资源门槛中 RSS 增长已确认不超限（门槛放宽至 2048 MiB）。
