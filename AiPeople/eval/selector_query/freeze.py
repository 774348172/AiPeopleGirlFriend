...
from eval.memory_scheduler.freeze import verify_manifest as verify_mem06_manifest


ROOT = Path(__file__).resolve().parents[2]
MANIFEST_PATH = ROOT / "eval" / "selector_query" / "recall01_contract_v1.json"
FROZEN_ASSET_PATHS = tuple(
    sorted(
        (
            "runtime/_selector_query.py",
            "eval/selector_query/freeze.py",
            "eval/selector_query/recall01_profile_v1.json",
            "eval/memory_scheduler/mem06_contract_v1.json",
            "设计文档/AI设计/当前权威设计/RelationshipRuntime_全局记忆选择器训练与模型决策方案_V2.md",
            "设计文档/AI设计/当前权威设计/RECALL-01_查询表示合同施工文档.md",
        )
    )
)
    ...
    *,
    ...
    manifest = {
        ...
        "upstream": {
            ...
            "manifest": "eval/memory_scheduler/mem06_contract_v1.json",
        },
        "scope": {
            "includes": "deterministic SelectorQuery construction, canonical serialization and hashing, bounded local BGE tokenization and one-shot normalized query encoding",
            "excludes": [
                "global exact similarity scan",
                "memory_id aggregation and Top32",
                "reranker",
                "model download or training",
            ],
            "next_checkpoint": "RECALL-02",
        },
...
def write_manifest(
    *,
    ...
    if verify_upstream:
        verify_mem06_manifest()
    if manifest_path.exists():
        raise FreezeVerificationError(
            "RECALL-01 contract manifest already exists and is immutable"
        )
...
def verify_manifest(
    *,
    ...
    if not manifest_path.is_file():
        raise FreezeVerificationError("RECALL-01 contract manifest is missing")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("contract_id") != "recall01-selector-query-representation-v1":
        raise FreezeVerificationError("unexpected RECALL-01 contract_id")
    freeze = manifest.get("freeze")
    if not isinstance(freeze, dict) or freeze.get("immutable") is not True:
        raise FreezeVerificationError("RECALL-01 manifest is not immutable")
    if freeze.get("manifest_sha256") != canonical_manifest_sha256(manifest):
        raise FreezeVerificationError("RECALL-01 manifest self SHA256 mismatch")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Freeze or verify RECALL-01 assets")
