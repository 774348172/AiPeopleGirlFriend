...
from eval.memory_materialization.build_assets import validate_assets
from eval.memory_propose.freeze import verify_manifest as verify_mem02_manifest


ROOT = Path(__file__).resolve().parents[2]
MANIFEST_PATH = ROOT / "eval" / "memory_materialization" / "mem03_contract_v1.json"
FROZEN_ASSET_PATHS = tuple(
    sorted(
        (
            "runtime/_memory_materialization.py",
            "eval/memory_materialization/build_assets.py",
            "eval/memory_materialization/freeze.py",
            "eval/memory_materialization/mem03_profile_v1.json",
            "eval/memory_materialization/fixtures/valid/materialization_cases_v1.json",
            "eval/memory_materialization/fixtures/invalid/materialization_cases_v1.json",
            "eval/memory_propose/mem02_contract_v1.json",
            "需求文档/项目框架需求.md",
            "设计文档/AI设计/当前权威设计/AI女友最小心智系统设计.md",
            "设计文档/AI设计/当前权威设计/MEM-03_确定性证据校验与物化施工文档.md",
        )
    )
)
    ...
    *,
    ...
    manifest = {
        ...
        "upstream": {
            ...
            "manifest": "eval/memory_propose/mem02_contract_v1.json",
        },
        "scope": {
            "includes": "ledger evidence verification, exact quote location, UTC normalization and proposed MemoryRepresentation materialization",
            "excludes": [
                "memory commit or status transition",
                "retry persistence",
                "vector projection",
                "background scheduler",
            ],
            "next_checkpoint": "MEM-04",
        },
...
def write_manifest(
    *,
    ...
    if verify_upstream:
        verify_mem02_manifest()
    validate_assets()
    if manifest_path.exists():
        raise FreezeVerificationError("MEM-03 contract manifest already exists and is immutable")
...
def verify_manifest(
    *,
    ...
    if not manifest_path.is_file():
        raise FreezeVerificationError("MEM-03 contract manifest is missing")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("contract_id") != "mem03-evidence-materialization-v1":
        raise FreezeVerificationError("unexpected MEM-03 contract_id")
    freeze = manifest.get("freeze")
    if not isinstance(freeze, dict) or freeze.get("immutable") is not True:
        raise FreezeVerificationError("MEM-03 manifest is not immutable")
    if freeze.get("manifest_sha256") != canonical_manifest_sha256(manifest):
        raise FreezeVerificationError("MEM-03 manifest self SHA256 mismatch")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Freeze or verify MEM-03 assets")
