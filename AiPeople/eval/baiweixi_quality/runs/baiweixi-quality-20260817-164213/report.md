...
> Run：`baiweixi-quality-20260817-164213`  
...
- 案例：`89`。
- 冻结种子尝试：`245`。
- 计划对话轮次：`301`；实际进入 Runtime：`293`。
- 判定：`{"pass": 52, "fail": 37}`。
- 原始机器归因：`{"character_failure": 33, "joint_or_ambiguous": 4}`。
- 审计主归因：`{"system_failure": 0, "character_failure": 1, "joint_or_ambiguous": 4, "evaluation_ambiguity": 32}`。
- Blocker 未通过：`26`。
- Important 通过率：`0.56`。
- Normal 通过率：`1.0`。
- 自动语义加权均分：`4.6886`。
...
| `heroine_state_continuity` | 10 | 9 | 1 | 0 |
| `identity_canon` | 12 | 5 | 7 | 0 |
| `memory_knowledge` | 2 | 1 | 1 | 0 |
| `protagonist_grounding` | 11 | 11 | 0 | 0 |
| `relationship_pacing` | 9 | 0 | 9 | 0 |
| `single_world` | 7 | 7 | 0 | 0 |
| `unknown_boundaries` | 9 | 4 | 5 | 0 |
| `world_canon` | 9 | 3 | 6 | 0 |
...
- `bwx.identity_canon.day_anchor`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 20260811]；歧义种子=[]
- `bwx.identity_canon.injury`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 314159, 20260811]；歧义种子=[]
- `bwx.world_canon.hidden_demons`：fail；审计归因=evaluation_ambiguity；失败种子=[42]；歧义种子=[]
- `bwx.world_canon.spiritual_energy`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 314159]；歧义种子=[]
- `bwx.world_canon.protagonist_origin`：fail；审计归因=evaluation_ambiguity；失败种子=[314159, 20260811]；歧义种子=[]
- `bwx.world_canon.home`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 314159]；歧义种子=[]
...
- `bwx.unknown_boundaries.accident`：fail；审计归因=evaluation_ambiguity；失败种子=[314159, 20260811]；歧义种子=[]
- `bwx.unknown_boundaries.organization`：fail；审计归因=evaluation_ambiguity；失败种子=[42]；歧义种子=[]
- `bwx.unknown_boundaries.protagonist_aura`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 314159, 20260811]；歧义种子=[]
- `bwx.unknown_boundaries.human_force`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 314159, 20260811]；歧义种子=[]
...
- `bwx.relationship_pacing.ownership`：fail；审计归因=evaluation_ambiguity；失败种子=[314159]；歧义种子=[]
- `bwx.relationship_pacing.chores`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 20260811]；歧义种子=[]
- `bwx.relationship_pacing.confession_pressure`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 314159, 20260811]；歧义种子=[]
- `bwx.relationship_pacing.leaving`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 314159, 20260811]；歧义种子=[]
- `bwx.relationship_pacing.gratitude`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 20260811]；歧义种子=[]
...
- `bwx.ability_limits.weather`：fail；审计归因=evaluation_ambiguity；失败种子=[314159, 20260811]；歧义种子=[]
- `bwx.ability_limits.memory_edit`：fail；审计归因=character_failure；失败种子=[314159, 20260811]；歧义种子=[]
- `bwx.ability_limits.instant_heal`：fail；审计归因=evaluation_ambiguity；失败种子=[314159]；歧义种子=[]
- `bwx.ability_limits.create_goods`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 314159, 20260811]；歧义种子=[]
- `bwx.ability_limits.mind_control`：fail；审计归因=evaluation_ambiguity；失败种子=[42, 20260811]；歧义种子=[]
- `bwx.ability_limits.aura_sense`：fail；审计归因=evaluation_ambiguity；失败种子=[42]；歧义种子=[]
