...
from eval.memory_commit.freeze import verify_manifest as verify_mem04_manifest


ROOT = Path(__file__).resolve().parents[2]
MANIFEST_PATH = ROOT / "eval" / "memory_vectors" / "mem05_contract_v1.json"
FROZEN_ASSET_PATHS = tuple(
    sorted(
        (
            "runtime/_memory_vectors.py",
            "runtime/migrations/006_memory_vectors.sql",
            "eval/memory_vectors/freeze.py",
            "eval/memory_vectors/mem05_profile_v1.json",
            "eval/memory_commit/mem04_contract_v1.json",
            "需求文档/项目框架需求.md",
            "设计文档/AI设计/当前权威设计/AI女友最小心智系统设计.md",
            "设计文档/AI设计/当前权威设计/RelationshipRuntime_全局记忆选择器训练与模型决策方案_V2.md",
            "设计文档/AI设计/当前权威设计/MEM-05_selector_view本地BGE向量增量失效与全量重建施工文档.md",
        )
    )
)
    ...
    *,
    ...
    manifest = {
        ...
        "upstream": {
            ...
            "manifest": "eval/memory_commit/mem04_contract_v1.json",
        },
        "scope": {
            "includes": "selector views, local BGE vectors, incremental invalidation, atomic full rebuild and contiguous matrix snapshot",
            "excludes": [
                "global similarity scan and Top32",
                "remote vector service",
                "online training",
                "background scheduler",
            ],
            "next_checkpoint": "MEM-06",
        },
...
def write_manifest(
    *,
    ...
    if verify_upstream:
        verify_mem04_manifest()
    if manifest_path.exists():
        raise FreezeVerificationError("MEM-05 contract manifest already exists and is immutable")
...
def verify_manifest(
    *,
    ...
    if not manifest_path.is_file():
        raise FreezeVerificationError("MEM-05 contract manifest is missing")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("contract_id") != "mem05-local-bge-vector-projection-v1":
        raise FreezeVerificationError("unexpected MEM-05 contract_id")
    freeze = manifest.get("freeze")
    if not isinstance(freeze, dict) or freeze.get("immutable") is not True:
        raise FreezeVerificationError("MEM-05 manifest is not immutable")
    if freeze.get("manifest_sha256") != canonical_manifest_sha256(manifest):
        raise FreezeVerificationError("MEM-05 manifest self SHA256 mismatch")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Freeze or verify MEM-05 assets")
