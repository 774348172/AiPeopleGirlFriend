...
app = FastAPI(title="Gemma 4 12B 60-turn review")
...
def _latest_run_dir() -> Path | None:
    values = sorted(
        (
            path
            for path in OUTPUT_ROOT.glob("gemma4_12b_60turn_*")
...
class ReviewStore:
    ...
    def resolve_run_dir(self) -> Path:
        ...
        if value is None or not (value / "report.json").is_file():
            raise HTTPException(status_code=404, detail="Gemma 4 12B 60轮报告尚未生成")
        return value
        ...
        turns.append(
            {
                ...
                "base": {
                    "reply": turn["normal"]["response"],
                    "elapsed_ms": turn["normal"].get("elapsed_ms"),
                    "first_output_ms": turn["normal"].get("first_output_ms"),
                    "first_visible_ms": turn["normal"].get("first_visible_ms"),
                    "eval_tokens_per_second": turn["normal"].get("eval_tokens_per_second"),
                    "thinking_chars": turn["normal"].get("thinking_chars"),
                    "manual_review": {},
                },
                "full_program": {
                    "reply": turn["thinking"]["response"],
                    "elapsed_ms": turn["thinking"].get("elapsed_ms"),
                    "first_output_ms": turn["thinking"].get("first_output_ms"),
                    "first_visible_ms": turn["thinking"].get("first_visible_ms"),
                    "eval_tokens_per_second": turn["thinking"].get("eval_tokens_per_second"),
                    "thinking_chars": turn["thinking"].get("thinking_chars"),
                    "manual_review": {},
                },
            }
        )
    model = report["arms"]["normal"]["model"]
    return {
        ...
        "speed_summary": {
            "base": report["summary"]["speed"]["normal"],
            "full_program": report["summary"]["speed"]["thinking"],
        },
        "phase_summary": [],
        ...
        "arm_labels": {
            "base": {
                "name": "A Gemma 4 12B 普通模式",
                "short_name": "A 普通模式",
                "detail": f"{model} · think=false · 最近6条消息 · 无动作旁白",
            },
            "full_program": {
                "name": "B Gemma 4 12B 思考模式",
                "short_name": "B 思考模式",
                "detail": f"{model} · think=true · 最近6条消息 · 无动作旁白",
            },
        },
        "turns": turns,
    }
...
async def report() -> FileResponse:
    return FileResponse(
        store.resolve_run_dir() / "report.json", filename="gemma4_12b_60turn_report.json"
    )
...
async def review_report() -> FileResponse:
    ...
    return FileResponse(store.review_path(), filename="gemma4_12b_60turn_user_review.json")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Gemma 4 12B 60-turn human review page")
