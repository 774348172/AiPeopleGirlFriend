...
app = FastAPI(title="Gemma 4 12B base versus Baiweixi QLoRA review")
...
def _audit_payload(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "arm_labels": {
            "base": {
                "name": "A Gemma 4 12B 官方基座 NF4",
                "short_name": "A 裸基座",
                "detail": "同一官方权重 · NF4 · Adapter关闭 · 普通模式 · 最近6条消息",
            },
            "full_program": {
                "name": "B Gemma 4 12B + 白未晞 QLoRA",
...
async def report() -> FileResponse:
    return FileResponse(store.resolve() / "report.json", filename="gemma4_base_qlora_60turn_report.json")
...
async def review_report() -> FileResponse:
    ...
    return FileResponse(store.review_path(), filename="gemma4_base_qlora_60turn_user_review.json")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Gemma base versus QLoRA 60-turn review page")
