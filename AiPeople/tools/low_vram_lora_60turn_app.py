...
def _latest_run_dir() -> Path | None:
    values = sorted(
        (path for path in OUTPUT_ROOT.glob("low_vram_lora_60turn_*") if (path / "report.json").is_file()),
...
class ReviewStore:
    ...
    def resolve_run_dir(self) -> Path:
        ...
        if value is None or not (value / "report.json").is_file():
            raise HTTPException(status_code=404, detail="低显存 LoRA 60轮报告尚未生成")
        return value
...
store = ReviewStore()
app = FastAPI(title="Low-VRAM Baiweixi LoRA 60-turn review")
...
def _audit_payload(report: dict[str, Any]) -> dict[str, Any]:
    thinking_requested = bool(report["controls"]["generation"].get("thinking"))

    def thinking_label(arm_key: str) -> str:
        if not thinking_requested:
            return "关闭思考"
        turns = report["arms"][arm_key]["turns"]
        actual = sum(bool(turn.get("thinking")) for turn in turns)
        return f"请求开启思考 · 实际思考 {actual}/{len(turns)} 轮"
    turns = []
        ...
        turns.append(
            {
                ...
                "base": {
                    "reply": turn["qwen35_9b"]["response"],
                    "elapsed_ms": turn["qwen35_9b"].get("elapsed_ms"),
                    "first_output_ms": turn["qwen35_9b"].get("first_output_ms"),
                    "first_visible_ms": turn["qwen35_9b"].get("first_visible_ms"),
                    "eval_tokens_per_second": turn["qwen35_9b"].get("eval_tokens_per_second"),
                    "thinking_chars": turn["qwen35_9b"].get("thinking_chars"),
                    "manual_review": {},
                },
                "full_program": {
                    "reply": turn["qwen3_14b"]["response"],
                    "elapsed_ms": turn["qwen3_14b"].get("elapsed_ms"),
                    "first_output_ms": turn["qwen3_14b"].get("first_output_ms"),
                    "first_visible_ms": turn["qwen3_14b"].get("first_visible_ms"),
                    "eval_tokens_per_second": turn["qwen3_14b"].get("eval_tokens_per_second"),
                    "thinking_chars": turn["qwen3_14b"].get("thinking_chars"),
                    "manual_review": {},
                },
            }
        )
    return {
        ...
        "speed_summary": {
            "base": report["summary"].get("speed", {}).get("qwen35_9b"),
            "full_program": report["summary"].get("speed", {}).get("qwen3_14b"),
        },
        "phase_summary": [],
        ...
        "arm_labels": {
            "base": {
                "name": f"A {report['arms']['qwen35_9b']['label']}",
                "short_name": "A 9B Q6 LoRA",
                "detail": f"合并白未晞 rank 8 LoRA · Q6_K · 最近6条消息 · {thinking_label('qwen35_9b')}",
            },
            "full_program": {
                "name": f"B {report['arms']['qwen3_14b']['label']}",
                "short_name": "B 14B Q4 LoRA v2",
                "detail": f"修复空思考监督 · Q4_K_M · 最近6条消息 · {thinking_label('qwen3_14b')}",
            },
        },
        "turns": turns,
    }
...
async def report() -> FileResponse:
    return FileResponse(store.resolve_run_dir() / "report.json", filename="low_vram_lora_60turn_report.json")
...
async def review_report() -> FileResponse:
    ...
    return FileResponse(store.review_path(), filename="low_vram_lora_60turn_user_review.json")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Low-VRAM Baiweixi LoRA 60-turn review page")
