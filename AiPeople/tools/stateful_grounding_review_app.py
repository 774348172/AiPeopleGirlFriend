...
STATIC_ROOT = Path(__file__).resolve().parent / "stateful_grounding_review_static"
...
app = FastAPI(title="Gemma 4 12B 多轮知识边界人工审核")
...
class Store:
    ...
    def resolve(self) -> Path:
        if self.run_dir is None or not (self.run_dir / "report.json").is_file():
            raise HTTPException(status_code=404, detail="多轮报告不存在")
        return self.run_dir
...
def public(value: dict[str, Any], total: int) -> dict[str, Any]:
    reviews = list(value.get("reviews", {}).values())
    return {
        **value,
        "completed": len(reviews),
        "total": total,
        "good": sum(item.get("quality") == "good" for item in reviews),
        "bad": sum(item.get("quality") == "bad" for item in reviews),
    }
...
async def review_report() -> FileResponse:
    ...
    if not store.path().is_file():
        store.write(value)
    total = len(store.report()["cases"])
    return FileResponse(store.path(), filename=f"gemma4_stateful_grounding{total}_user_review.json")
...
async def report() -> FileResponse:
    total = len(store.report()["cases"])
    return FileResponse(
        store.resolve() / "report.json",
        filename=f"gemma4_stateful_grounding{total}_report.json",
    )


@app.put("/api/user-review/{ordinal}")
async def save_review(ordinal: int, payload: dict[str, Any]) -> JSONResponse:
    report = store.report()
    if ordinal < 1 or ordinal > len(report["cases"]):
