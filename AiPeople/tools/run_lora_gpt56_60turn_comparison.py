...
LORA_MODEL = "baiweixi-7b-fix:latest"
...
async def _run_lora(
    ...
    async with httpx.AsyncClient(
        ...
        if LORA_MODEL not in digests:
            raise RuntimeError(f"Ollama model is not installed: {LORA_MODEL}")
        for turn in inputs:
            ...
            generated = await paired._generate(
                ...
                prompt=prompt,
                seed=seed,
                request_id=f"lora-gpt56-60:lora:turn-{ordinal:02d}",
            )
            ...
            print(
                json.dumps(
                    {"turn": ordinal, "arm": "lora", "response": reply},
                    ensure_ascii=False,
                ),
                flush=True,
            )
    return (
        {
            ...
            "label": "当前 LoRA Q4",
            "model": LORA_MODEL,
            "ollama_digest": digests[LORA_MODEL],
            "surface": "Ollama local raw ChatML",
            "turns": turns,
        },
        history,
    )
...
async def _run(args: argparse.Namespace) -> int:
    ...
    lora_arm, _ = await _run_lora(inputs, args.ollama_url)
    gpt_arm = _build_gpt_arm(inputs, gpt_responses)
    report = {
        "schema_version": 1,
        "scope": "stateful_lora_q4_vs_gpt56sol_codex_internal_60turn",
        ...
        "arms": {"lora": lora_arm, "gpt": gpt_arm},
        "turns": [],
    }
    for input_turn, lora_turn, gpt_turn in zip(
        inputs, lora_arm["turns"], gpt_arm["turns"], strict=True
    ):
    ...
    report["summary"] = {
        ...
        "lora_mean_elapsed_ms": _mean_elapsed(lora_arm["turns"]),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run LoRA vs internal GPT 60-turn test")
