...
app = FastAPI(title="GLM low-VRAM 60-turn review")
...
def _latest_run_dir() -> Path | None:
    runs = sorted(
        (
            path
            for path in OUTPUT_ROOT.glob("glm_low_vram_60turn_*")
...
class ReviewStore:
    ...
    def resolve_run_dir(self) -> Path:
        run_dir = self.run_dir or _latest_run_dir()
        if run_dir is None or not (run_dir / "report.json").is_file():
            raise HTTPException(status_code=404, detail="GLM 60轮报告尚未生成")
        return run_dir
...
def _arm_payload(value: dict[str, Any]) -> dict[str, Any]:
    return {
        "reply": value.get("response") or "[本轮没有有效回复]",
    ...
    turns = [
        {
            ...
            "base": _arm_payload(turn["glm4_9b_0414_q6"]),
            "full_program": _arm_payload(turn["glm46v_flash_q4"]),
        }
        for turn in report["turns"]
    ]
    arm_a = report["arms"]["glm4_9b_0414_q6"]
    arm_b = report["arms"]["glm46v_flash_q4"]
    return {
        ...
        "speed_summary": {
            "base": report["summary"]["speed"]["glm4_9b_0414_q6"],
            "full_program": report["summary"]["speed"]["glm46v_flash_q4"],
        },
        "phase_summary": [],
        "source_report": str(store.resolve_run_dir() / "report.json"),
        "has_prior_review": store.review_path().is_file(),
        "run_enabled": False,
        "speed_context_note": "同一工作站 · 同一冻结60轮输入 · 两侧均为本次流式运行",
        "speed_latency_note": "首字为玩家可见回答首字；热启动统计排除各侧第一轮冷加载",
        "arm_labels": {
            "base": {
                "name": "A GLM-4-9B-0414 Q6_K 裸基座",
                "short_name": "A GLM 9B Q6",
                "detail": f"{arm_a['model']} · 最近6条各自历史 · 普通模式 · 无动作旁白",
            },
            "full_program": {
                "name": "B GLM-4.6V-Flash Q4_K_M 裸基座",
                "short_name": "B GLM 4.6V Q4",
                "detail": f"{arm_b['model']} · 纯文本模式 · 最近6条各自历史 · 无动作旁白",
            },
        },
        "turns": turns,
    }
...
async def report() -> FileResponse:
    return FileResponse(
        ...
        filename="glm_low_vram_60turn_report.json",
    )
...
async def review_report() -> FileResponse:
    ...
    return FileResponse(
        store.review_path(), filename="glm_low_vram_60turn_user_review.json"
    )


@app.put("/api/user-review/{turn_number}")
async def save_user_review(
    turn_number: int, payload: dict[str, Any]
) -> JSONResponse:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="GLM low-VRAM 60-turn review page")
