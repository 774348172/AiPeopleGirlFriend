...
def _latest_run_dir() -> Path | None:
    values = sorted(
        (
            path
            for path in OUTPUT_ROOT.glob("qwen35_9b_q6_gpt56_60turn_*")
...
class ReviewStore:
    ...
    def resolve_run_dir(self) -> Path:
        ...
        if value is None or not (value / "report.json").is_file():
            raise HTTPException(status_code=404, detail="Qwen3.5 / GPT 60 轮报告尚未生成")
        return value
...
store = ReviewStore()
app = FastAPI(title="Qwen3.5-9B Q6 vs GPT-5.6-sol 60-turn review")
        ...
        turns.append(
            {
                ...
                "base": {
                    "reply": turn["qwen35"]["response"],
                    "elapsed_ms": turn["qwen35"].get("elapsed_ms"),
                    "manual_review": {},
                },
                "full_program": {
    ...
    return {
        ...
        "arm_labels": {
            "base": {
                ...
                "short_name": "Qwen3.5-9B Q6",
                "detail": "qwen3.5-9b-q6:latest · 本地 Ollama · 原生模板 · 关闭思考 · 最近 6 条消息",
            },
            "full_program": {
                "name": "GPT-5.6-sol",
...
async def report() -> FileResponse:
    return FileResponse(
        ...
        filename="qwen35_9b_q6_gpt56_60turn_report.json",
    )
...
async def review_report() -> FileResponse:
    ...
    return FileResponse(
        store.review_path(), filename="qwen35_9b_q6_gpt56_60turn_user_review.json"
    )
...
async def save_user_review(turn_number: int, payload: dict[str, Any]) -> JSONResponse:
    ...
    if base_quality not in {"good", "bad"}:
        raise HTTPException(status_code=422, detail="请选择 Qwen3.5-9B Q6 好或差")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Qwen3.5-9B Q6 vs GPT-5.6-sol review page")
