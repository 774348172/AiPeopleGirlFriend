...
def _latest_run_dir() -> Path | None:
    values = sorted(
        (
            path
            for path in OUTPUT_ROOT.glob("qwen35_27b_gpt56_60turn_*")
...
store = ReviewStore()
app = FastAPI(title="Qwen3.5-27B vs GPT-5.6-sol 60-turn review")
    ...
    return {
        ...
        "arm_labels": {
            "base": {
                ...
                "detail": "qwen3.5:27b · 本地 Ollama · 原生模板 · 关闭思考 · 最近 6 条消息",
            },
            "full_program": {
                "name": "GPT-5.6-sol",
...
async def report() -> FileResponse:
    return FileResponse(
        ...
        filename="qwen35_27b_gpt56_60turn_report.json",
    )
...
async def review_report() -> FileResponse:
    ...
    return FileResponse(
        store.review_path(), filename="qwen35_27b_gpt56_60turn_user_review.json"
    )
...
async def save_user_review(turn_number: int, payload: dict[str, Any]) -> JSONResponse:
    ...
    if base_quality not in {"good", "bad"}:
        raise HTTPException(status_code=422, detail="请选择 Qwen3.5-27B 好或差")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Qwen3.5-27B vs GPT-5.6-sol review page")
