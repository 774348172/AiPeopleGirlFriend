# Apple M3 训练包 Qwen3.5 —— 秦未晞 (Qwen3.5-4B LoRA)
...
> **本包（2026-08-07）与 v5 的区别：基座升级为 Qwen3.5-4B**（数据与 v5 完全一致：矩阵 119 + 新批次 997 + P1-P4 验证样本 48 + 纠错 41）。
> ⚠ **Qwen3.5-4B 是多模态（VLM）架构**：LLaMA-Factory 侧已实测可训（8G 显存 QLoRA，视觉塔自动冻结）；**MLX 侧需 mlx-lm 支持 Qwen3_5ForConditionalGeneration**——首次训练如报架构不支持，先 `pip install -U mlx-lm`。模板已确认兼容（im_start 体系 + nothink 渲染）。**诊断基线注意**：换基座后 diagnostic120 的 base 对照对象变了，需重建基线再对比。**
...
| 文件 | 条数 | 口径 | 说明 |
|---|---|---|---|
| `data/qin_v4_matrix.jsonl` | 119 | **候选（T12 矩阵全通过版）** | 六类行为矩阵：支持陪伴 42 / 正典问答 21 / 含糊回应 18 / 记忆纠正 15 / 安静陪伴 12 / 关系边界 11。⚠ 人工复核未完成（v2 阶段 0 冻结：`human_review_incomplete`） |
| `data/qin_v4_1000_final.jsonl` | 997 | **候选（T15 新批次）** | 批量生成：casual 489 / romance 305 / emotion 169 / identity 12 / protective 22。⚠ v2 阶段 0 冻结：`candidate_unreviewed`（未经 v2 标准验收，不得视为 release） |
| `data/qin_v4_p2_general.jsonl` | 24 | **新正典（P2）** | 常识/逻辑问答（三杯水/算账/换算），数学人设自嘲但正常作答——防"数学白痴泛化" |
| `data/qin_v4_p1_safety_final.jsonl` | 18 | **新正典（P1）** | 安全/急救场景（脑卒中/入眼/溺水/煤气/触电/过敏/惊厥/骨折/烫伤/冻伤），处置动作经 G8 硬门验证 |
| `data/qin_v4_p3_casual.jsonl` | 6 | **新正典（P3）** | 反朗读抽查样本（属性按话题触发词放行） |
...
> ⚠ **v2 阶段 0 冻结状态（2026-08-07）**：本包数据按《通用数据生成器施工总计划_v2.md》阶段 0 冻结。
> 997/119 均为 **候选数据**（`candidate_unreviewed` / `human_review_incomplete`），未经 v2 晋升标准（§8.2 七条）验收，
> 也未完成人工复核 —— **"已生成/已导出" ≠ "已发布"**。阶段 0 完成前：不得以此作为正式训练数据来源，不得继续扩大数据规模。

> **生成方式**：`python 01_prepare_data.py --new-only`（合并矩阵 119 + 新批次 997；不带 `--new-only` 会按 v2 逻辑取存量 2530，本包已删除存量文件，必须用 `--new-only`）。
> **训练步数**：02_train.sh 按 M3 路线冻结规则自动算 `iters = 1.5 × N`（train 1093 条 → 1640 iters，M3 Max 约 45-65 分钟）。
...
```bash
mlx_lm.generate --model Qwen/Qwen3.5-4B --adapter-path adapters/qinweixi  # 阶段 5：基座统一（freeze03） \
...
#    → data/train.jsonl (1093条) + data/valid.jsonl (130条)

# 3) 训练（MLX 首次会自动下载 Qwen3.5-4B 并转 MLX 格式，约 8GB）
./02_train.sh
#    → adapters/qinweixi/*.safetensors + adapter_config.json
#    预计: M3 Max 约 45-65 分钟（1640 iters）；it 自动 = 1.5 × N
...
| `--iters 1640` | 自动 = 1.5×N | M3 路线冻结规则（train 1093 条 → 1640）；过拟合时减半 |
| `--max-seq-len 2048` | 2048 | 覆盖全部样本（最长 ~1076 字符），省显存 |

预计时长：M3 Max 约 45-65 分钟；M3 基础款约 1.5-2 小时（v5）。
...
ollama run qinweixi
# VRAM: Qwen3.5-4B Q4_K_M ≈ 2.5GB，3060 8G 无压力
```
...
2. **MLX 首次下载模型**：`mlx_lm.lora` 首次会从 HF 下载 Qwen3.5-4B 并转换，需要网络 + ~10GB 磁盘；慢可设 `export HF_ENDPOINT=https://hf-mirror.com`。
