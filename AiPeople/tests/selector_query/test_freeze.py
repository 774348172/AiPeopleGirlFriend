...
from eval.selector_query.freeze import (
    FROZEN_ASSET_PATHS,
...
def test_recall01_frozen_chain_verifies() -> None:
    manifest = verify_manifest()
    assert manifest["scope"]["next_checkpoint"] == "RECALL-02"
    assert manifest["upstream"]["contract_id"] == (
        "mem06-background-scheduling-isolation-v1"
    )
