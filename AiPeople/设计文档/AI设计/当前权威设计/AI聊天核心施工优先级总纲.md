...
`SYS-12S` 前台协议修正及首轮正式长跑已经完成。当前必须先完成后台协议和发布调度收敛，再继续 `WMR-08 / SYS-12` 的正式冻结：

```text
冻结后台 HTTP 400、R1 失败和 M2 第 55 轮证据
→ 回答后整理与五分钟整理改用短 Patch 和确定性最小投影
→ 请求前强制校验 Prompt 与保留输出的上下文预算
→ R1 改为短合同并由 Runtime 编译正式记忆提案
→ 合并后台任务、限制重试并建立队列背压
→ 隔离 GPU 环境重跑同一 60 轮与一小时发布栈
→ 继续白未晞角色质量、正式检索工件和发布性能冻结
→ 通过后冻结 P0
```
...
原因：实时世界、记忆隔离、后台整理、崩溃恢复和投影重建基础已经存在，但后续合并式 `short_semantic_v1 / M1` 把最终回复塞入短 Patch JSON，真实交互已经证明会导致长度贴边、重复叙事和角色名损坏。该问题必须先在 Runtime 主干修正，旧 60 轮结果不能替代新协议验收。合成第二角色只用于隔离测试，不能视为第二正式女主角。
...
`SYS-12S` 是对已发现错误前台合同的必要修正，不是新增第二套 Runtime。2026-08-14 已完成前台最小输入施工及正式 60 轮/一小时发布栈复测：M2 Prompt 在长跑中保持 `548～630` token，`GAME_REPLY` 保持 `359～482` token，证明前台最小输入方向有效；事务、冻结快照、故障恢复和多女主隔离通过。但 1 轮 M2 协议失败，后台整理 Prompt 增长至接近 `4096` 上下文并大量 HTTP 400，五分钟整理 0 次成功，R1 失败 30 个，队列峰值 28，资源增长和系统 GPU 门槛失败。下一步唯一施工入口调整为“后台整理最小协议 + R1 短合同 + 队列合并/背压 + 隔离资源复测”，通过后再继续 `WMR-08 / SYS-12` 和 P0 冻结；角色质量仍独立保持未通过。

2026-08-14 已完成上述入口的代码施工和隔离 GPU 正式复验：冻结负载持续 `3636.56s`，59 轮提交、1 轮 M2 协议失败；B1/R2 未再出现结构截断、HTTP 400 或合同失败，R2 作业 59/59 完成，队列峰值 10 并最终排空，崩溃恢复、投影重建、快照一致性和多女主隔离通过。`SYS-11 / SYS-12 / P0` 仍未通过，剩余阻塞为 M2 非法 Patch 导致整轮失败、进程树稳态段 RSS 增长约 `1382.77 MiB`，以及组合 GPU 峰值 `5336 MiB` 超过门槛 `216 MiB`。下一唯一施工入口调整为“M2 安全降级/容错 + RSS 增长定位 + 组合 GPU 降载”，不得回退已验证通过的 B1/R2、背压、账本、快照、隔离和恢复合同。
