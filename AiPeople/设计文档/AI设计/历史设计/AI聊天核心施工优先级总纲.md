...
- 白未晞正式回复候选工件：**通过**（重训 7B Q4，SHA256 `df6eefdc…`，判官口径零失败）。
- 正式 Embedding 与 Reranker：**通过**（SYS-09，20/20，P95 43ms）。
- 白未晞正式质量集：已冻结（manifest `2bb53297…`）并完成完整 89 案例复验；判官零失败，34 个机器失败中 33 个为词表误杀（判定口径：以判官为准，2026-08-19 决策）、1 个真实失败待补样本；4 个人工长会话评审完成（3 pass + 1 issue 宽松口径）。
- 白未晞结构化输出稳定性、角色硬正典、主客体、未知边界和评测器可靠性：**通过**（M2 容错 + 长测 60/60 + 判官零失败）。
- 正式样本 P95、模型切换、KV cache 拆分、组合显存和一小时真实稳定性：长测通过（每轮 p50 1186ms / p95 1495ms）；8GB 目标卡实测延期（预验证 num_gpu=20 可行）。
- **P0 正式冻结：已冻结（2026-08-19）**，冻结合同 `eval/world_mind_p0/wmr08_p0_freeze_contract_v1.json`。已知延期项：8GB 目标卡实测、3 个真实失败样本（protagonist_job/species/age）。
